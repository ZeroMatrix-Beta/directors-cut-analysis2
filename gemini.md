# The Director's Cut Protocol: Real Analysis Transcription & Refinement Blueprint (V14)
...
- **Chronology Lock:** **Preserve the exact chronological order of speech and board actions. Do not reorder content for clarity. The protocol is a transcript, not a textbook.**
...
- **Environment Separation Rule:** **Each semantic environment must serve exactly one role. Do not merge narration, derivation, and commentary inside the same block unless explicitly required.**
- **Fallback for the Illegible:** If a board state is completely illegible and the formula is not dictated verbally, do not hallucinate the math or attempt to guess based on poor OCR. Use the placeholder `\textbf{[Illegible formula]}` inside the `math_stroke` environment, accompanied by a brief description of what you can see.
- **Failure Condition:** **Any omission of a spoken sentence, board element, or derivation step constitutes a protocol failure. When uncertain, include rather than omit.**
