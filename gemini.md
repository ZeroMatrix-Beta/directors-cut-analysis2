...
- **Environment Separation Rule & Intentional Redundancy:** **Each semantic environment must serve exactly one role. Do not merge narration, derivation, and commentary inside the same block unless explicitly required. In particular, derivations must be placed in `math-stroke`, and summary reasoning must be placed in `explanation-of-steps`, not in `spoken-clean`. Mathematical content may be repeated across environments if it aids clarity. NEVER hesitate to formally restate a formula, mathematical setup, or conclusion inside a `math-stroke` block, even if the professor just dictated it verbally in the preceding `spoken-clean` block. This intentional redundancy ensures the `math-stroke` block functions as a complete, highly readable textbook derivation.**
