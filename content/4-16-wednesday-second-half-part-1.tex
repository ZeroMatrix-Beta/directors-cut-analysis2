...
\begin{nicebox}[Formal Proof Setup: Differentiation Under the Integral Sign]
...
\begin{proof}[Proof of Differentiation Under the Integral Sign]

\textbf{Step 1: Formulating the Difference Quotient}
\begin{spokenclean}[00:00:09 - 00:01:20]
...
\begin{mathstroke}[The Incremental Quotient]
...
\begin{spokenclean}[00:01:20 - 00:02:10]
To do this, we will use the Mean Value Theorem, right? By the Mean Value Theorem, we know that the incremental quotient (i.e., $\frac{f(x, t_0 + h) - f(x, t_0)}{h}$) is equal to the partial derivative at some intermediate point. Let's call this intermediate point $\xi_x$, noting that it depends strictly on $x$. If we assume that $h$ is positive, just for simplicity, then this $\xi_x$ belongs to the open interval $(t_0, t_0 + h)$.
\end{spokenclean}

\begin{mathstroke}[MVT Substitution]
...
\begin{spokenclean}[00:03:00 - 00:06:18]
...
\begin{mathstroke}[Vanishing Error Term]
...
\textbf{Explanation of Steps (The Power of Uniformity):}
By bounding the error term $E(x)$ uniformly across the entire domain $K$, we guarantee that the "worst-case" deviation of the difference quotient from the true partial derivative is still just $\varepsilon$. Because the spatial volume $\mu(K)$ is finite and fixed, shrinking $h$ squeezes this worst-case error volume perfectly to zero, leaving only the exact integral of the partial derivative.
\end{mathstroke}
\end{proof}

\begin{spokenclean}[00:06:18 - 00:08:45]
...
\begin{spokenclean}[00:08:45 - 00:12:07]
...
\begin{mathstroke}[Approximating the Open Set]
\begin{align*}
...
\begin{spokenclean}[00:12:07 - 00:15:00]
...
So, if $U$ is the union of all $K_\ell$ (i.e., $U = \bigcup_{\ell=0}^\infty K_\ell$), then the integral over $U$ of $f$ is exactly the same as the limit, when $\ell$ goes to infinity, of the integral of $f$ over $K_\ell$. In our definition, we are taking the supremum over a massive collection of all possible Jordan measurable compact sets. But in practice, it is enough to take just one increasing sequence of compact sets that exhausts your open set. You can easily construct sequences like this using cubes.

Why is the limit of the sequence the same as the supremum? Because if you give me any fixed compact set $K$, and I have a sequence of sets whose interiors eventually cover everything, then $K$ will be covered by a finite subcover (i.e., $K \subset \bigcup_{i=0}^N \text{int}(K_i) \subset K_N$).
\end{spokenclean}

\begin{mathstroke}[Finite Subcover Argument]
...
\textbf{Explanation of Steps (Topological Exhaustion):}
This is a beautiful interplay between topology and calculus. The abstract definition of an improper integral requires checking an infinite, uncountably massive collection of compact subsets $K$. But the topological property of compactness guarantees that any such $K$ will be completely swallowed by our expanding sequence $K_\ell$ at some finite index $N$. Therefore, simply taking the limit of one sequence naturally captures the supremum over all sets.
\end{mathstroke}
\end{proof}
