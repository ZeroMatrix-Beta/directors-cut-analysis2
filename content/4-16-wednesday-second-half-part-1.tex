...
To do this, we will use the Mean Value Theorem, right? By the Mean Value Theorem, we know that the incremental quotient (i.e., $f(x, t_0 + h) - f(x, t_0)$ divided by $h$) is equal to the partial derivative at some intermediate point. Let's call this intermediate point $\xi_x$, noting that it depends strictly on $x$. If we assume that $h$ is positive, just for simplicity, then this $\xi_x$ belongs to the open interval $(t_0, t_0 + h)$.
\end{spoken_clean}
