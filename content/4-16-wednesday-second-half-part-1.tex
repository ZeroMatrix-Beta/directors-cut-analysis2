% --- V11 CUSTOM ENVIRONMENTS DEFINITIONS ---
\newtcolorbox{nice-box}[1][]{title=#1, colback=white, colframe=black!80, fonttitle=\bfseries, arc=2mm}
\newtcolorbox{orangeformula}[1][]{colback=orange!5, colframe=orange!80!black, title=#1, fonttitle=\bfseries, arc=2mm}

\newtcolorbox{spoken_clean}[1][]{colback=blue!5, colframe=blue!70!black, title=\textbf{\textsf{Speech [#1]}}, arc=2mm, boxrule=0.5mm, before skip=1em, after skip=1em}

\newtcolorbox{math_stroke}[1][]{colback=green!5, colframe=green!60!black, title=\textbf{\textsf{Board Action [#1]}}, arc=2mm, boxrule=0.5mm, before skip=1em, after skip=1em}

\newenvironment{didactic_insight}[1][]
  {\vspace{1em}\noindent\textbf{\textsf{Didactic Insight [#1]:}}\par\noindent\itshape}
  {\par\vspace{1em}}

\newenvironment{meta_note}[1][]
  {\vspace{1em}\noindent\textsf{\textbf{Meta Note [#1]:}} \ignorespaces}
  {\par\vspace{1em}}
...
\begin{nice-box}[Board State \& Formal Proof Setup: Differentiation Under the Integral Sign]
...
The professor applies the Mean Value Theorem (MVT) to the integrand for each fixed $x$:
\[
...
\begin{meta_note}[Pedagogical Transition]
...
\begin{nice-box}[Board State \& Formal Definition: Improper Integrals]
...
\begin{equation}
\underbrace{\int_U f(x) \, dx}_{\text{``integral over the whole open set''}} = \sup \left\{ \underbrace{\int_K f}_{\text{``bigger than over any compact subset''}} \mid \underbrace{K \subset U}_{\text{``approximate inside''}}, K \text{ cpt J.m.} \right\}
\end{equation}
\end{math_stroke}

\begin{nice-box}[Board State \& Formal Proposition: Exhaustion by Compact Sets]
...
\begin{meta_note}[System]
Output length limit reached. The highly polished 15-minute sequence is complete. Please prompt "Continue" for the next segment (15:00 - 30:00).
\end{meta_note}
