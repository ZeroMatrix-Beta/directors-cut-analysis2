...
\begin{tikzpicture}[scale=1.5]
    % Axes
    \draw[->, thick, ForestGreen] (0,0) -- (4,0) node[below right] {$x \in \mathbb{R}^k$};
    \draw[->, thick, ForestGreen] (0,0) -- (0,3) node[above left] {$y \in \mathbb{R}^{n-k}$};
    ...
    % Slice at x
    \draw[BrickRed, thick] (2, 0.5) -- (2, 2.8); % The vertical slice
    \node[BrickRed, below] at (2, 0.5) {$x$};
\end{tikzpicture}
\end{center}
...
\begin{color-box}{BurntOrange}[Cavalieri's Principle for Jordan Measurable Sets]
\begin{theorem}[Cavalieri's Principle for Jordan Measurable Sets]
\label{thm:cavalieri-jordan-sets}
...
\begin{tikzpicture}[scale=1.5]
    % Axes
    \draw[->, thick, ForestGreen] (0,0) -- (4,0) node[below right] {$x$};
    \draw[->, thick, ForestGreen] (0,0) -- (0,3) node[above left] {$y$};
    ...
    \draw[fill=yellow!20, draw=ForestGreen, thick] (1,1) -- (1,2) -- (2,2) -- (2,1) -- (2.5,1) -- (2.5,0.5) -- (3,0.5) -- (3,1.5) -- (2.5,1.5) -- (2.5,2.5) -- (1.5,2.5) -- (1.5,1) -- cycle;

    % Outer approximation (red/light red)
    \draw[fill=red!10, draw=BrickRed, thick, dashed] (0.5,0.5) -- (0.5,2.8) -- (3.8,2.8) -- (3.8,0.5) -- cycle;
    \draw[draw=BrickRed, thick] (0.8,0.8) rectangle (3.2,2.7); % Outer boundary simplified for visual clarity

    % Labels for slices
    \node at (0.7, 2.7) {\small $A$};
    \node[BrickRed, right] at (3.8, 2.7) {$2^{-p}$}; % Pixel size
...
\[
\int_{[-c,c]^k} f(x) \, dx = (2^{-p})^n \cdot (\text{number of dyadic cubes in } F) = \mu_n(F)
\]
And similarly for $g(x)$:
\[
\int_{[-c,c]^k} g(x) \, dx = (2^{-p})^n \cdot (\text{number of dyadic cubes in } G) = \mu_n(G)
\]
...
\begin{spoken-clean}[00:16:22 - 00:16:32]
So, now we will do an extension of this of this theorem. That is for for functions. It will actually be an immediate consequence of this theorem.
\end{spoken-clean}
...
\begin{tikzpicture}[scale=1.5]
    ...
    \node[BurntOrange] at (2.2,2.8,1) {Surface $z = f(x,y)$};
\end{tikzpicture}
\end{center}
\begin{explanation-of-steps}
