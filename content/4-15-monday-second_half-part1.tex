\section{Monday, April 15th: Second Half of the Lecture \\
...
\begin{meta_note}[Previous Board State]
...
\begin{spoken_clean}[00:00:08 - 00:02:20]
...
\begin{nice-box}[Board State 1: The Potato and the Slice]
The professor moves to a clean section of the left board. He draws a large, smooth, blob-like shape (the ``potato'') and labels it $A$. He draws the Cartesian coordinate axes, explicitly defining the X-axis as $\mathbb{R}^k$ and the Y-axis as $\mathbb{R}^{n-k}$. He draws a dashed bounding box around $A$. He then picks a specific point $x$ on the horizontal axis, draws a vertical line segment exactly through $A$, and projects that 1D slice onto the Y-axis, labeling it $A_x$.
\end{nice-box}
...
\begin{spoken_clean}[00:02:20 - 00:04:50]
...
\begin{nice-box}[Board State 2: Formalizing the Slices]
The professor moves to the right board to formalize the geometric picture into strict mathematical notation. He writes the definition of the slice $A_x$ and the upper/lower marginal densities.
\end{nice-box}

\begin{math_stroke}[Formal Definition of the Slice and Marginals]
...
\begin{redundant_explanation}[Inner and Outer Guarantees]
Even if a solid body $A$ is perfectly measurable, a specific 1D slice could theoretically intersect a fractal boundary or a cloud of points, making that exact slice non-measurable. To bypass this entirely, the professor rigorously defines the marginals using the universally safe inner and outer measures.
...
\begin{spoken_clean}[00:04:50 - 00:07:30]
...
So this integral is just you summing up the $\overline{\varphi}$. The $\overline{\varphi}$ is the measure of each slice! Now, you can tell me, ``Maybe for some slices this measure is not well-defined because the outer and the inner measure do not coincide!'' I tell you: do not worry! Just put either the outer or the inner. I don't care, because when you integrate the outer, or the inner, or anything in between, you will always recover the exact measure of your set $A$. 

Okay, let's prove that. To do the proof, I will do another drawing.
\end{spoken_clean}

\begin{orangeformula}[Cavalieri's Principle]
...
\begin{spoken_clean}[00:07:30 - 00:11:00]
...
\begin{nice-box}[Board State 3: The Pixelated Sandwich]
The professor moves to a new board. He redraws the smooth blob $A$. Using yellow chalk, he meticulously draws a jagged, pixelated boundary entirely inside $A$ (the inner approximation $F$). Using red chalk, he draws a larger pixelated boundary entirely enclosing $A$ (the outer approximation $G$). He draws a vertical line to show how the 1D slice of $F$ is a subset of the slice of $A$, which is a subset of the slice of $G$.
\end{nice-box}

\begin{math_stroke}[Geometric Proof Setup: Inner and Outer Approximations]
\begin{center}
\begin{tikzpicture}[scale=1.5]
    % Axes
...
\begin{spoken_clean}[00:11:00 - 00:15:15]
...
\begin{nice-box}[Board State 4: Formalizing the Dyadic Cubes]
The professor moves to the far right board to set up the rigorous notation for the dyadic cubes (the ``pixels''). He shows how an $n$-dimensional cube factors smoothly into a Cartesian product of a $k$-dimensional cube and an $(n-k)$-dimensional cube, matching the $X$ and $Y$ axis splitting perfectly.
\end{nice-box}

\begin{math_stroke}[Dyadic Cube Definitions]
...
\begin{didactic_insight}[Calculus via Combinatorics]
