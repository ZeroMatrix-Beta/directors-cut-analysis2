# The Director's Cut Protocol: Transcription & Refinement Blueprint (V1.19-FVT - Full Video Transcription)
...
- **Edge Cases & Protocol Meta-Rules**
  ...
  - **CRITICAL (Snapshot vs. Delta - Active Board Scope):** Treat every `math-stroke` block as a completely self-contained visual snapshot of the *currently active mathematical derivation or chalkboard panel*. NEVER treat a new `math-stroke` as a "delta" that only outputs new information. If a list, derivation, or diagram spans a continuation, you MUST completely restate all previously written items/equations *relevant to that specific, active context* in the new `math-stroke` to maintain structural anchoring. A 'snapshot' strictly encompasses the *logical content being actively developed or referenced*. Do not endlessly duplicate content from *other, physically visible but no longer actively referenced or modified* chalkboards that the lecturer has left behind.
  - **Global State Checkpoint (Long-Form Cognitive Anchor):** To maintain logical consistency and focus over the full duration of the video, you MUST inject a periodic "Global State Checkpoint" using the `\begin{ai-global-state-checkpoint-invisible-content}` scratchpad.
